...
const profileSchema = object({
  full_name: string().required(t('common.validator.empty.profile.full_name')),
  email: string()
  ...
  >
    <div class="p-4 bg-white rounded-[30px] flex flex-col lg:p-7">
      ...
      <UForm :schema="profileSchema" ref="formRef" :state="form">
        ...
        >
          <UFormGroup
            name="full_name"
            class="mt-5"
            :ui="{ error: 'ml-[156px] mt-2 text-red-500 dark:text-red-400' }"
            v-slot="{ error }"
          >
            ...
            >
              <USelectMenu
                ...
                :ui-menu="{
                  ...
                  background: 'bg-white',
                  base: 'absolute md:min-w-[350px] max-md:min-w-[250px] overflow-x-hidden overflow-y-auto scroll-py-1',
                  padding: 'p-0',
                  option: {
              ...
              >
                <template #option="{ option }">
                  <div class="flex flex-row gap-[10px]">
                    <img :src="option.image" class="w-[22px] h-[20px]" alt="" />
                    <div class="text-12-500-20">{{ option.name }}</div>
                  </div>
                </template>
                <div class="border border-r-0 py-[11px] rounded-l-[49px] pl-4 pr-3 flex flex-row gap-[10px] w-[75px]">
                  <img class="w-[22px] h-[20px]" :src="getCountryFlag(form.country_code)" />
      ...
      <div class="flex justify-between mt-[28px]">
        <div class="text-[#7A7D89] text-[12px] manrope font-normal leading-[20px] flex items-center justify-center">
          Last edit: {{ showLastEdit }}
        </div>
        <UButton
          :class="
            isDisableButton
              ? 'text-[#d6d8e5] !bg-[#a4a8b8] hover:bg-[#a4a8b8]'
              : 'text-white !bg-[#FF5524] hover:bg-[#FF5524]'
          "
