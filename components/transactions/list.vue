...
<template>
  <div v-if="!transactionList.length" class="flex flex-col items-center justify-center gap-4 p-4 md:p-10 pt-0 h-full">
    <img
      class="min-w-[150px] md:min-w-[200px] min-h-[150px] md:min-h-[200px]"
      src="/images/transactions/no-transaction.svg"
      alt=""
    />
  ...
  >
    ...
    <template #amount-data="{ row }">
      <div class="text-14 md:text-16-700-24 w-[150px] min-w-[100px] text-[#1C1D23] text-center">
        {{
          row.transaction_vc_type === TransactionVCType.TOP_UP || row.transaction_vc_type === TransactionVCType.REFUND
    ...
    <template #status-data="{ row }">
      <div
        ...
        :style="{ color: getStatusColor(row?.transaction_vc_status)?.color }"
      >
        ...
        <div
          ...
          :style="{ background: getStatusColor(row?.transaction_vc_status)?.color }"
        ></div>
      </div>
    </template>
