    ...
    >
      ...
      <template #amount-data="{ row }">
        <div class="text-14 md:text-16-700-24 w-[150px] min-w-[100px] text-[#1C1D23] text-center">
          {{
            row.transaction_vc_type === TransactionVCType.TOP_UP || row.transaction_vc_type === TransactionVCType.REFUND ? `+${formatMoney(row?.received_amount)}` : `-${formatMoney(row?.received_amount)}`
          }}
        </div>
      </template>
